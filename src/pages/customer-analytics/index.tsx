...
import { Alert, Box, Card, CardContent, InputAdornment, MenuItem, Select, TextField } from '@mui/material'
...
import { cityData, fetchAnalyticsCustomer, fetchAnalyticsGraphCustomer } from 'src/api'
...
const CustomerAnalytics = () => {
  ...
  const anylytcisdata = async () => {
    ...
    // payload.startDate = "2023/02/10"
    // payload.endDate = "2023/05/27"
    // payload.address_id = selectedCityId || 0

    payload.address_id = 0

    // payload.address_id = 0
    // payload.company_id = auth?.user?.company?.id || 0
    // payload.team_member = selectedTeamId || 0

    // console.log(auth?.user?.company?.id, '----auth?.user?.company?.id')

    try {
      const resp = await fetchAnalyticsCustomer(payload)
  ...
  const graphData = async () => {
    ...
    payload.year = selectedYear || 2023

    // payload.year = selectedYear
    // payload.property_id = selectedProjId || 0
    // payload.user_id = auth?.user?.id
    // payload.company_id = auth?.user?.company?.id

    // payload.startDate = "2023/02/10"
    // payload.endDate = "2023/04/27"

    try {
      const resp = await fetchAnalyticsGraphCustomer(payload)
      if (resp?.code == 200) {
        console.log(resp, '------resp graph')
        const graphData = resp?.data[2]?.LineChart?.stats.map((item: any) => {
          return item?.Onboarding_Trend
        })

        const piechartstats = resp?.data[0]?.PieChart.map((item: any) => {
          return item?.stats
        })
        const removePercentage = piechartstats.map((percent: any) => parseInt(percent.replace('%', ''), 10))
        const piechartTitle = resp?.data[0]?.PieChart.map((item: any) => {
          return item?.title
        })

        const barchartstats = resp?.data[1]?.BarChart.map((item: any) => {
          return item?.stats
        })
        const barchartTitle = resp?.data[1]?.BarChart.map((item: any) => {
          return item?.title
        })
                  ...
                  >
                    {true && (
                      <div style={{ position: 'relative' }}>
                        <label className='block my-2 text-sm font-medium text-blueText lg:text-md '>Filter</label>
                        <div style={{ position: 'relative', width: '230px', marginRight: '10px' }}>
                          ...
                          {selectedValueCity && searchValue != '' && (
                            ...
                            >
                              {cityDataDetails?.data != undefined &&
                                cityDataDetails?.data?.length > 0 &&
                                cityDataDetails?.data.map((d: any, ind: any) => {
                                  return (
                                    <div
                                      key={ind}
                                      onClick={() => {
                                        selectedCity(d?.name, d?.id)
                                      }}
              ...
              <Grid>
                <Grid item xs={12}>
                  ...
                  <Grid>
                    ...
                    <ApexAreaChart heading={`Onboarding Trend`} tooltipText='Leads' data={lineCHartData} />
                  </Grid>
                </Grid>
              </Grid>
            </CardContent>
                  ...
                  <Table sx={{ minWidth: 700 }} aria-label='customized table'>
                    <TableHead>
                      <TableRow>
                        ...
                        <StyledTableCell align='center'>Total Contacted</StyledTableCell>
                        <StyledTableCell align='center'>Total Visit</StyledTableCell>
                        <StyledTableCell align='center'>Total Finalized</StyledTableCell>
                      </TableRow>
                    </TableHead>
                    <TableBody>
                      {tableData.map((row: any) => (
                        <StyledTableRow key={row.Sr_No}>
                          ...
                          <StyledTableCell align='center'>{row.Total_Contacted}</StyledTableCell>
                          <StyledTableCell align='center'>{row.Total_Visit}</StyledTableCell>
                          <StyledTableCell align='center'>{row.Total_Finalized}</StyledTableCell>
                        </StyledTableRow>
                      ))}
                    </TableBody>
                  </Table>
