...
const ForgotPassword = () => {
  ...
  const schema = Yup.object().shape({
    email: Yup.string().strict(false).trim().email('Enter valid email.').required('Email is required.'),
    ...
    otp: Yup.string().min(6, 'Please enter 6 digit OTP').max(6).required('OTP is required.')
  })
  const [resetForm, setResetForm] = useState<boolean>(false)
  const [minutesEmail, setMinutesEmail] = useState(0)
  const [secondsEmail, setSecondsEmail] = useState(59)

  // const [values, setValues] = useState<requiredFields>(defaultValues)
  ...
  const { otp: otpCheck } = getValues()

  const [disableEmailOtpBtn, setDisableEmailOtpBtn] = useState<boolean>(true)

  const handleEmailOtpBtn = () => {
    ...
    if (formErrors.email) {
      dispatch(resetFormErors('email'))
    }
    if (email.length > 0) {
      setDisableEmailOtpBtn(false)
    } else {
      setDisableEmailOtpBtn(true)
    }
    if (emailOtp) {
      dispatch(
        removeOtpLoaders({
  ...
  useEffect(() => {
    return () => {
      dispatch(resetForm1())
      dispatch(removeOtpLoaders({ verificationType: 1 }))
    }
  ...
  const { loginLoader: loader, emailLoading, emailOtp, resetLoading } = useSelector((state: any) => state.user)

  const onSubmit = (data: requiredFields) => {
    const { email, newPassword, confirmPassword, otp } = data
    dispatch(
      resetPassword({
        data: { email, newPassword, confirmPassword, otp },
      ...
      <RightWrapper sx={skin === 'bordered' && !hidden ? { borderLeft: `1px solid ${theme.palette.divider}` } : {}}>
        ...
        >
          <BoxWrapper>
            ...
            <Box sx={{ mb: 6 }}>
              <TypographyStyled variant='h5'>{resetForm ? 'Reset Password' : 'Forgot Password? 🔒'}</TypographyStyled>
            ...
            <form onSubmit={handleSubmit(onSubmit)}>
              <>
                  ...
                  <Controller
                    ...
                    render={({ field: { value, onChange, onBlur } }) => (
                      <TextField
                        ...
                        error={Boolean(errors.email || formErrors.email)}
                      />
                    )}
                  />
                ...
                {!resetForm && (
                  <LoadingButton
                    disabled={Boolean(errors.email) || disableEmailOtpBtn}
                    fullWidth
                    size='large'
                    style={{ padding: '15px' }}
                    variant='outlined'
                    loading={loader || emailLoading}
                    onClick={sendOtp}
                    sx={{ mb: 7 }}
                  >
                    Send OTP
                      ...
                      render={({ field: { value, onChange } }) => (
                        <TextField
                          ...
                          onChange={e => {
                            ...
                            if (formErrors?.emailOTP || formErrors?.otp) {
                              dispatch(resetFormErors('emailOTP'))
                              dispatch(resetFormErors('otp'))
                            }
                            if (e.target.value.length === 6) {
                              const { email } = getValues()
                              dispatch(
                          ...
                          InputProps={{
                            endAdornment: (
                              <InputAdornment position='end'>
                                {emailOtp && <CheckboxMarkedCircleOutline color='success' />}
                                {!emailLoading && !emailOtp ? (
                                  <Timer
                                    minutes={minutesEmail}
                                    seconds={secondsEmail}
                                    setMinutes={setMinutesEmail}
                                    setSeconds={setSecondsEmail}
                                  />
                                ) : null}
                                {emailLoading && <CircularProgress thickness={3} size={25} />}
                                {minutesEmail === 0 && secondsEmail === 0 && <Button onClick={sendOtp}>Resend</Button>}
                              </InputAdornment>
                            )
                          }}
                          error={Boolean(errors.otp || formErrors.otp || formErrors?.emailOTP)}
                          autoComplete='off'
                          type='number'
                        />
                      )}
                    />
                    {Boolean(errors.otp || formErrors.emailOTP || formErrors.otp) && (
                      <FormHelperText sx={{ color: 'error.main' }}>
                        {errors?.otp?.message || formErrors.emailOTP || formErrors.otp}
                      </FormHelperText>
                    )}
                  </FormControl>
                </>
                ...
                <Controller
                  ...
                  render={({ field: { value, onChange } }) => (
                    <OutlinedInput
                      ...
                      onChange={e => {
                        onChange(e)
                        if (watch('confirmPassword') === e.target.value) {
              ...
              <FormControl fullWidth sx={{ mt: 4 }}>
                ...
                <Controller
                  ...
                  render={({ field: { value, onChange } }) => (
                    <OutlinedInput
                      ...
                      id='validation-basic-confirmPassword'
                      error={Boolean(errors.confirmPassword || formErrors.confirmPassword)}
                      inputProps={{
                        autoComplete: 'new-password12312121',
                        form: {
                          autoComplete: 'off'
                        }
                      }}
                ...
                {(errors.confirmPassword || formErrors.confirmPassword) && (
                  <FormHelperText sx={{ color: 'error.main' }} id='validation-basic-confirmPassword'>
              ...
              <LoadingButton
                ...
                variant='contained'
                sx={{ mb: 5.25, mt: 4 }}
                disabled={!isValid || emailLoading || Object.keys(formErrors)?.length > 0 || otpCheck?.length === 0}
