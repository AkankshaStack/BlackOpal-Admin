 ...
 * @param currentURL
 */
export const hasActiveChild = (item: NavGroup, currentURL: string,query : any =undefined): boolean => {
  const { children } = item
  if (!children) {
    return false
  }
  ...
  for (const child of children) {
    ...
    if (
      ...
      childPath &&
      currentURL &&
      (childPath === currentURL || (currentURL.includes(childPath) && childPath !== '/') || (query && childPath.includes(query)) )
    ) {
      return true
    }
  }
